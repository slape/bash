#A script to check if one or more devices is offline.
...
if (( $currentDevices == 0 )); then
  ...
  mail -s  "$server has a device offline" $email <<< "One or more devices has gone offline for QTS server $server"
fi
